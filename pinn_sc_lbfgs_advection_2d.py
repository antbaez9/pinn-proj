...
class PhysicsInformedNN():
    ...
    def net_u(self, x, y, t):
        ...
        if args.quantity == "momentum":
            ...
            if t.shape[0] == 100:
                integral_u_dx = torch.sum(self.dnn(t_by_xy)*self.delta_xy**2, dim=(1,2))
            ...
            # use integral on mesh to find current momentum of system
            if t.shape[0] == 100:
                integral_u_dx = torch.sum(self.dnn(t_by_xy)**2*self.delta_xy**2, dim=(1,2))
            ...
            second_term_momentum = integral_u_dx / volume_xy

            # use integral on mesh to find current energy of system
            if t.shape[0] == 100:
                integral_u_dx_2 = torch.sum(self.dnn(t_by_xy)**2*self.delta_xy**2, dim=(1,2))
            else:
                n_batches = 5
                batch_size = int(t.shape[0]/n_batches)
                ...
                for i in range(0, t.shape[0], batch_size):
                    end_idx = min(i + batch_size, t.shape[0])
                    integrand_2 = self.dnn(t_by_xy[i:end_idx, :, :])**2
                    integral_u_dx_2[i:end_idx, :] = torch.sum(integrand_2*self.delta_xy**2, dim=(1,2))

            second_term_energy = integral_u_dx_2 / volume_xy

            c_momentum_tensor = (torch.full(x.shape, self.c_momentum)/volume_xy).to(device)
            c_energy_tensor = (torch.full(x.shape, self.c_energy)/volume_xy).to(device)

            # print(torch.mean((second_term_momentum - c_momentum_tensor) ** 2), torch.mean((second_term_energy - c_energy_tensor) ** 2))
    ...
    def loss_func(self):
        ...
        u_pred, loss_c = self.net_u(self.x_u, self.y_u, self.t_u)
