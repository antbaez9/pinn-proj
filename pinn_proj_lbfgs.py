        ...
        if not args.proj:
            ...
            if args.quantity == "momentum":
                ...
                if args.eq == "react-diff":
                    ...
                    projection = torch.sqrt(approx_c.unsqueeze(1)/integral_u_dx).to(device)
                ...
                # energy
                if t.shape[0] == 100:
                    integral_u_dx_2 = torch.sum((self.dnn(t_by_x)-second_term.unsqueeze(1).repeat(1, n, 1))**2, dim=1).to(device)
                else:
                    integral_u_dx_2 = []
                    n_batches = 5
                    batch_size = int(t.shape[0]/n_batches)
                    for i in range(0, t.shape[0], batch_size):
                        integral_part_2 = torch.sum((self.dnn(t_by_x[i:i+batch_size, :, :])-second_term[i:i+batch_size, :].unsqueeze(1).repeat(1, n, 1))**2, dim=1).to(device)
                        integral_u_dx_2.append(integral_part_2)
                ...
                if args.eq == "react-diff":
                    ...
                    approx_c_momentum = nearest_c_momentum + self.grad_c_momentum[t_index_closest] * delta_t.squeeze(1)
                    approx_c_energy = nearest_c_energy + self.grad_c_energy[t_index_closest] * delta_t.squeeze(1)

                    projected_u = (u - second_term)*torch.sqrt((approx_c_energy.unsqueeze(1)/delta_x - (approx_c_momentum.unsqueeze(1)**2)/(n*delta_x**2))/integral_u_dx_2)+approx_c_momentum.unsqueeze(1)/volume_x
                    ...
                    projected_u = (u - second_term)*torch.sqrt((c_energy_tensor/delta_x - (c_momentum_tensor**2)/(n*delta_x**2))/integral_u_dx_2)+c_momentum_tensor/volume_x
...
# if args.proj:
#     torch.save(model.dnn.state_dict(), f'../graphing/data/pinn_proj_{args.eq}_{args.quantity}.pth')
# else:
#     torch.save(model.dnn.state_dict(), f'../graphing/data/pinn_{args.eq}_{args.quantity}.pth')
...
if args.quantity == "momentum" or args.quantity == "energy":
    ...
    # if args.proj:
    #     torch.save(c_pred, f'../graphing/data/c_pred_proj_{args.eq}_{args.quantity}.pth')
    #     if args.quantity == "momentum":
    #         torch.save(c_momentum, f'../graphing/data/c_true_proj_{args.eq}_momentum.pth')
    #     elif args.quantity == "energy":
    #         torch.save(c_energy, f'../graphing/data/c_true_proj_{args.eq}_energy.pth')

    # else:
    #     torch.save(c_pred, f'../graphing/data/c_pred_pinn_{args.eq}_{args.quantity}.pth')
